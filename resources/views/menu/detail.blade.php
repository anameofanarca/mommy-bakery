        ...
        <div class="flex flex-col h-full justify-between">
            ...
            <form action="{{ route('cart.add', $product->id) }}" method="POST" class="mt-8 flex flex-wrap items-center gap-4" x-data="{ quantity: 1 }">
                ...
                <div class="flex items-center border border-gray-200 rounded-xl overflow-hidden bg-white shadow-sm h-12">
                    ...
                    <input type="text" x-model="quantity" name="qty" class="w-12 text-center text-sm font-semibold focus:outline-none h-full bg-transparent" readonly>
                    
                    <button type="button" @click="quantity++" class="px-5 hover:bg-gray-50 text-gray-600 text-sm font-bold transition-colors border-l border-gray-100 h-full flex items-center justify-center">+</button>
                </div>
